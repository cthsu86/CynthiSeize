...
function checkAssumption(rw,n)
  ...
  elseif n<11 && n>=7 && rw<.5
    warning('CIRCSTAT:circ_wwtest:sampleSize6x11AndVectorTooShort', ...
        'Test not applicable. Average number of samples per population 6 < x < 11 and average resultant vector length < 0.5.');
  elseif n>=5 && n<7 && rw<.55
    warning('CIRCSTAT:circ_wwtest:sampleSize4x7AndVectorTooShort', ...
        'Test not applicable. Average number of samples per population 4 < x < 7 and average resultant vector length < 0.55.');
  elseif n < 5
...
function [alpha, idx, w] = processInput(varargin)
  ...
  elseif nargin==2
    ...
    if ~(size(idx,1)==size(alpha,1))
      error('CIRCSTAT:circ_wwtest:InputSizeMismatch', 'Input dimensions do not match: alpha and idx must be the same size.');
    end
    w = ones(size(alpha));  
  elseif nargin==3
    alpha = varargin{1}(:);
    ...
    if ~(size(w,1)==size(alpha,1))
      error('CIRCSTAT:circ_wwtest:InputSizeMismatch', 'Input dimensions do not match: alpha and w must be the same size.');
    end  
  else
    error('CIRCSTAT:circ_wwtest:TooManyInputs', 'circ_wwtest takes between 2-4 arguments. Type help circ_wwstat.')
  end
end
