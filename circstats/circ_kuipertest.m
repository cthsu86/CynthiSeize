...
% Make sure we grab the right table
kupierpath = fullfile(fileparts(mfilename('fullpath')), 'kuipertable.mat');
try
    kFile = load(kupierpath);
catch
    error('CIRCSTAT:circ_kuipertest:tableMissing', 'circ_kuipertest requires a table of pre-computed values (kuipertable.mat), in the same directory as this file.');
end
...
if ~easy
   ...
   if row == 0
       error('CIRCSTAT:circ_kuipertest:NOutOfRange', 'N too small: alpha1 and alpha2 must contain 5+ values each.');
   else
      warning('CIRCSTAT:circ_kuipertest:nNotFound', ...
          'N=%d not found in table, using closest N=%d present.',n,nn(row));
   end
end

% find minimal p-value and test-statistic
idx = find(kFile.ktable(row,2:end) < k, 1, 'last');
if ~isempty(idx)
  p = alpha(idx);
else
  p = 1;
end
K = kFile.ktable(row,idx+1);
end
